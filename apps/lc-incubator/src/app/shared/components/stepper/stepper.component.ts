...
import { QuestionnaireStep, QuestionsService } from '../../services/questions.service'
...
export class StepperComponent implements OnInit, OnDestroy {
  ...
  configurationLength$: Observable<number> = this.questionService.getSteps().pipe(map((x) => x.length))
  configuration$: Observable<QuestionnaireStep[]> = this.answersService.getAnswers().pipe(
    switchMap((answers) => {
        ...
        map((steps) => {
          ...
          steps.forEach((step) => {
              ...
              if (question.condition) {
                ...
                if (quest) {
                  if (quest.type !== 'checkbox') {
                    const answer = answers.find((a) => a.questionId === quest.id)
                    if (answer) {
                      isHidden = !condition.value.includes(answer.value ?? '')
                    }
                  } else {
                    const answer = answers.find((a) => a.questionId === quest.id)
                    if (answer) {
                      isHidden = !answer.values?.some((a) => condition.value.includes(a))
                    }
                  }
                }
              }
            ...
            const answersForNonHiddenQuestionsInStep = answers.filter((a) => {
              const question = step.questions.find((q) => q.id === a.questionId)
              return question && !question.isHidden
            })

            if (answersForNonHiddenQuestionsInStep.length === step.questions.filter((q) => !q.isHidden).length) {
              step.completed = true
            } else {
