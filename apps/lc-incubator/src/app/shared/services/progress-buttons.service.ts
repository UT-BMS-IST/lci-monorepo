...
export class ProgressButtonsService {
  ...
  private previousClicked$: BehaviorSubject<any | null> = new BehaviorSubject(
    null
  );
  ...
  shouldShowNext() {
    return this.questionsService.getSteps().pipe(
      map((items) => items.length + 2), //+1 because of result step and answers step
      switchMap((length) =>
        this.questionsService
          .getCurrentStep()
          .pipe(map((step) => step < length - 1))
      )
    );
  }
  ...
  shouldShowRestart() {
    return this.questionsService.getSteps().pipe(
      map((items) => items.length + 2), //+1 because of result step
      switchMap((length) =>
        this.questionsService
          .getCurrentStep()
          .pipe(map((step) => step === length - 1))
    ...
    return this.answerService.getAnswers().pipe(
      switchMap((answers) => {
        return combineLatest([
          this.questionsService.getCurrentStep(),
          ...
          map(([step, steps]) => {
            ...
            const answersForNonHiddenQuestionsInStep = answers.filter((a) => {
              const question = currentStep.questions.find(
                (q) => q.id === a.questionId
              );
            ...
            return (
              ...
              currentStep.questions.filter((q) => !q.isHidden).length
            );
          })
        );
      })
